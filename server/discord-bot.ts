...
const QR_IMAGE_PATH = path.join(process.cwd(), "attached_assets/qr_j2y_2026.jpg");
...
if (interaction.isStringSelectMenu() && interaction.customId === 'select_items_for_order') {
  ...
  return;
    ...
    // Handle button admin_detail - Konfirmasi (logic berbeda untuk Item vs PTPT)
    if (interaction.isButton() && interaction.customId === 'admin_detail') {
      const hasRole = isAdmin(message.member);
      const member = interaction.member as any;
      const hasRole = member?.roles?.cache?.has(ALLOWED_ROLE_ID);
      ...
      try {
        // Check order type dari activeOrders
        const orderSession = activeOrders.get(interaction.channel?.id || '');
        const isItemOrder = orderSession?.orderType === 'item';
